...
function App() {
    const defaultTrækprocent = "38";


    const [beløb, setBeløb] = useState("");
    const [trækprocent, setTrækprocent] = useState(defaultTrækprocent);
    const [message, setMessage] = useState(false);

  const defertBeløb = useDeferredValue(beløb)

    const sum =
    (+defertBeløb - +defertBeløb * 0.08) * (+trækprocent / 100) + +defertBeløb * 0.08;

    const beløbBetal = Math.ceil(sum)

    const beløbUdBetalt = Math.ceil(+defertBeløb - beløbBetal)
    ...
    return (
        <main className="grid place-content-center h-screen gap-2 px-2">
            ...
            <div className="text-green-700">
                Jeg få udbetalt på konto: {beløbUdBetalt} kr.
            </div>
